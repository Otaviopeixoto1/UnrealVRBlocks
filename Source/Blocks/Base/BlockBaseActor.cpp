...
ABlockBaseActor *ABlockBaseActor::GetHighlightCopy()
{

	if (GetWorld())
	{
		ABlockBaseActor *copyActor = GetWorld()->SpawnActor<ABlockBaseActor>(ABlockBaseActor::StaticClass(), GetTransform());

		copyActor->BlockBaseComponent->DestroyComponent();
		for (auto& Elem : blocks) 
		{
			// Create new BlockBaseComponent:
			UBlockBaseComponent* copyComponent = NewObject<UBlockBaseComponent>(copyActor);
			copyComponent->SetStaticMesh(Elem.Key->GetStaticMesh());
			copyComponent->AttachToComponent(copyActor->RootComponent, FAttachmentTransformRules::KeepRelativeTransform);

			//FTransform elemTransf = blocks[Elem.Key].ToFTransform();
			// blockTransform = blocks[Elem.Key].ToFTransform() * actorToThisTransform;

			copyComponent->SetRelativeTransform(Elem.Key->GetRelativeTransform());
			copyComponent->RegisterComponent();
			copyComponent->voxelVolume = Elem.Key->voxelVolume;
			//copyComponent->OnComponentBeginOverlap.AddDynamic(copyActor, &ABlockBaseActor::OnOverlapBegin);
			//copyComponent->OnComponentEndOverlap.AddDynamic(copyActor, &ABlockBaseActor::OnOverlapEnd);
			copyComponent->SetCollisionProfileName(FName("NoCollision"));

			//FVector v = blockTransform.GetLocation();

			copyActor->blocks.Add(copyComponent, FBlockTransform(Elem.Key->GetRelativeTransform()));
		}
